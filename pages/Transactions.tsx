...
import { Plus, Trash2, Camera, Loader2, Search, Filter, Calendar, ChevronLeft, ChevronRight, TrendingUp, TrendingDown, PiggyBank, FileText, FileSpreadsheet, Download } from 'lucide-react';
...
const Transactions: React.FC = () => {
  ...
  const exportPDF = () => {
    ...
    const savingsColor = monthlyStats.savings >= 0 ? [79, 70, 229] : [245, 158, 11]; // Indigo or Amber
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      // If user adds a transaction for a different month, optionally switch view? 
      // For now, we stay on current view.
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2 w-full lg:w-auto">
            <button 
                onClick={exportCSV}
                className="flex-1 lg:flex-none justify-center bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 px-4 py-2.5 rounded-xl flex items-center shadow-sm transition-all font-medium"
            ...
            <button 
                ...
                className="flex-1 lg:flex-none justify-center bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl flex items-center shadow-lg shadow-indigo-600/20 transition-all active:scale-95 font-medium"
            >
      ...
      <div className="bg-white rounded-2xl p-4 border border-slate-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="flex items-center bg-slate-50 rounded-xl p-1 w-full md:w-auto">
          ...
          <div className="px-6 font-bold text-slate-700 w-40 text-center select-none">
        ...
        <div className="flex gap-6 w-full md:w-auto justify-around md:justify-end">
           <div className="text-center md:text-right">
             ...
             <span className="text-emerald-600 font-bold flex items-center justify-center md:justify-end gap-1">
               <TrendingUp className="w-3 h-3" /> ${monthlyStats.income.toFixed(0)}
             </span>
           </div>
           ...
           <div className="text-center md:text-right">
             ...
             <span className="text-rose-600 font-bold flex items-center justify-center md:justify-end gap-1">
               <TrendingDown className="w-3 h-3" /> ${monthlyStats.expense.toFixed(0)}
             </span>
           </div>
           <div className="w-px bg-slate-100 h-10 hidden md:block"></div>
           <div className="text-center md:text-right">
             <span className="text-xs text-slate-400 font-semibold uppercase block">Savings</span>
             <span className={`font-bold flex items-center justify-center md:justify-end gap-1 ${monthlyStats.savings >= 0 ? 'text-indigo-600' : 'text-amber-600'}`}>
               <PiggyBank className="w-3 h-3" /> ${monthlyStats.savings.toFixed(0)}
             </span>
           </div>
        </div>
      </div>
        ...
        <div className="flex gap-4 mb-6 bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            <div className="relative flex-1">
                ...
                <input 
                    ...
                    className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-slate-700"
                />
            </div>
            {/* Optional: Add generic filter button UI (functionality handled by search for now) */}
          ...
          <div className="overflow-x-auto">
            ...
            {filteredData.length === 0 && (
              <div className="p-12 text-center text-slate-400 bg-slate-50/30">
                ...
                <p className="text-sm mt-1">No activity found for {formatMonthYear(currentDate)}.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
            <button
              ...
              className="w-full py-4 border-2 border-dashed border-indigo-200 rounded-xl bg-white text-indigo-600 hover:bg-indigo-50 hover:border-indigo-300 transition-all flex flex-col items-center justify-center gap-2 group"
            >
              {isScanning ? (
                <>
                ...
                <>
                  ...
                  <div className="text-center">
                    <span className="font-bold block">Scan Receipt with AI</span>
                    <span className="text-xs text-indigo-400">Auto-fill details from image</span>
                  </div>
                </>
              )}
            </button>
          ...
          <div className="grid grid-cols-2 gap-4 p-1 bg-slate-100 rounded-xl">
            <label className="cursor-pointer">
              ...
              />
              <div className="text-center py-2.5 rounded-lg text-sm font-semibold text-slate-500 peer-checked:bg-white peer-checked:text-rose-600 peer-checked:shadow-sm transition-all">
                Expense
              </div>
            </label>
            <label className="cursor-pointer">
              <input 
                ...
                className="hidden peer"
              />
              <div className="text-center py-2.5 rounded-lg text-sm font-semibold text-slate-500 peer-checked:bg-white peer-checked:text-emerald-600 peer-checked:shadow-sm transition-all">
                Income
              </div>
            </label>
          </div>
          ...
          <div className="space-y-4">
            <div>
                ...
                <div className="relative">
                    ...
                    value={form.amount} 
                    onChange={handleInputChange}
                    className="w-full pl-8 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none font-bold text-lg text-slate-900"
                    required 
                    />
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    value={form.date} 
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none text-slate-700 font-medium"
                    required 
                    />
                </div>
                ...
                <div>
                    ...
                    value={form.category} 
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none text-slate-700 font-medium appearance-none"
                    >
                    {form.type === 'expense' 
          ...
          <button 
            type="submit"
            disabled={isSaving}
            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-xl transition-all shadow-xl shadow-indigo-600/20 active:scale-[0.98] mt-4 flex justify-center items-center disabled:opacity-70 disabled:active:scale-100"
          >
